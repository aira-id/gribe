...
// SherpaOnnxOnlineRecognizer is a wrapper for sherpa-onnx OnlineRecognizer
// This would be the actual sherpa-onnx type when the library is available
type SherpaOnnxOnlineRecognizer interface {
	IsReady(stream interface{}) bool
	Decode(stream interface{})
	GetResult(stream interface{}) *SherpaOnnxResult
}

// SherpaOnnxOnlineStream is a wrapper for sherpa-onnx OnlineStream
type SherpaOnnxOnlineStream interface {
	AcceptWaveform(sampleRate int, samples []float32)
}

// SherpaOnnxResult represents the result from sherpa-onnx
type SherpaOnnxResult struct {
	Text string
}

// SherpaOnnxASRProvider implements the ASRProvider interface using sherpa-onnx
type SherpaOnnxASRProvider struct {
...
// NewSherpaOnnxASRProvider creates a new sherpa-onnx ASR provider
func NewSherpaOnnxASRProvider(config *domain.TranscriptionConfig) (*SherpaOnnxASRProvider, error) {
	if config == nil {
		config = &domain.TranscriptionConfig{
			Model:    "zipformer",
			Language: "en",
	...
	provider := &SherpaOnnxASRProvider{
		config:          config,
...
func (p *SherpaOnnxASRProvider) initializeRecognizer() error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *SherpaOnnxASRProvider) Transcribe(ctx context.Context, audio []byte, config *domain.TranscriptionConfig) (<-chan domain.TranscriptionChunk, error) {
...
func (p *SherpaOnnxASRProvider) TranscribeStream(ctx context.Context, config *domain.TranscriptionConfig) (chan<- []byte, <-chan domain.TranscriptionChunk, error) {
...
func (p *SherpaOnnxASRProvider) GetSupportedModels() []string {
	return p.supportedModels
}

// GetSupportedLanguages returns list of supported language codes
func (p *SherpaOnnxASRProvider) GetSupportedLanguages() []string {
	return p.supportedLangs
}

// Close releases any resources held by the provider
func (p *SherpaOnnxASRProvider) Close() error {
	p.mu.Lock()
	defer p.mu.Unlock()
